...
const messages = {
      ...
      ERROR_WORKBENCH_NOT_FOUND:
            "Neon Dreams could not find the workbench HTML file. This is likely due to a change in VS Code's internal structure. Please open an issue on the Synthwave '84 GitHub repository to report this.",
...
function activate(context) {
      ...
      let disableGlow =
            config && config.disableGlow ? !!config.disableGlow : false;

      let brightness =
            parseFloat(config.brightness) > 1
                  ? 1
                  : parseFloat(config.brightness);
            ...
            function () {
                  ...
                  if (!workbenchPaths) {
                        vscode.window.showErrorMessage(
                              messages.ERROR_WORKBENCH_NOT_FOUND
                        );
                        return;
                  }
                  ...
                  try {
                        ...
                        if (!isEnabled) {
                              ...
                              let output = html.replace(
                                    /^.*(<!-- SYNTHWAVE QUANTUM --><script src="quantum.js"><\/script><!-- NEON DREAMS -->).*\n?/gm,
                                    ""
                              );
                              ...
                              output = html.replace(
                                    /\<\/html\>/g,
                                    `	<!-- SYNTHWAVE QUANTUM --><script src="quantum.js"></script><!-- NEON QUANTUM -->\n`
                              );
                              output += "</html>";
      ...
      let disposableNebulosa = vscode.commands.registerCommand(
            "NebulosaSynthwave.enableNeon",
            function () {
                  const appDir = path.dirname(vscode.env.appRoot);
                  const base = path.join(appDir, "app", "out", "vs", "code");
                  ...
                  try {
                        // generate production theme JS
                        const chromeStyles = fs.readFileSync(
                              __dirname + "/css/editor_chrome-nebulosa.css",
                              "utf-8"
                        );
                        const jsTemplate = fs.readFileSync(
                              __dirname + "/js/theme_template.js",
      ...
      let disable = vscode.commands.registerCommand(
            "QuantumSynthwave.disableNeon",
            uninstall
      );

      context.subscriptions.push(disposable);
      context.subscriptions.push(disposableNebulosa);
...
function uninstall() {
      ...
      try {
            ...
            if (isEnabled) {
                  ...
                  vscode.window
                        .showInformationMessage(messages.DEACTIVATED, {
                              title: "Restart editor to complete",
                        })
                        .then(function (msg) {
                              vscode.commands.executeCommand(
